...
main = tkinter.Tk()
main.title("Heart Disease Prediction using Machine Learning Algorithms")
...
def preprocessDataset():
    ...
    label = dataset.groupby('Heart_disease').size()
    ...
    label.plot(kind="bar")
    plt.title("Number of Normal & Heart Disease Instances in dataset")
    plt.show()
...
def predict():
    ...
    for i in range(len(predict)):
        ...
        if predict[i] == 1:
            text.insert(END,"Test Data = "+str(dataset[i])+" PREDICTED AS ====> Expected Heart Disease in Future\n\n")    
    

font = ('times', 15, 'bold')
title = Label(main, text='Heart Disease Prediction using Machine Learning Algorithms', justify=LEFT)
title.config(bg='maroon', fg='#FFFFFF')  
...
font1 = ('times', 12, 'bold')
loadButton = Button(main, text="Upload Heart Disease Dataset", command=loadDataset)
